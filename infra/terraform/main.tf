...
resource "aws_security_group" "instance" {
  ...
  description = "SSM-only access for trnsparse CI"
  vpc_id      = var.vpc_id
...
resource "aws_instance" "ci" {
  ...
  user_data = <<-EOF
    ...
    set -euxo pipefail
    cd /home/ubuntu
    sudo -u ubuntu git clone https://github.com/trnsci/trnsparse.git trnsparse
    ...
    sudo -u ubuntu $NEURON_VENV/bin/pip install -e '/home/ubuntu/trnsparse[dev]'
  EOF
