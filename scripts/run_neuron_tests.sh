#!/usr/bin/env bash
#
# Run neuron-marked pytest tests on the trnsparse CI instance.
#
# Usage:
#   AWS_PROFILE=aws ./scripts/run_neuron_tests.sh [instance_type]
#
# Default instance_type is trn1 (looks for Name=trnsparse-ci-trn1).
...
if [[ "$WARM" == "1" ]]; then
  PYTEST_INVOCATION="\$NEURON_VENV/bin/pytest /home/ubuntu/trnsparse/tests/ -v -s -m neuron --tb=short && echo === WARM PASS === && \$NEURON_VENV/bin/pytest /home/ubuntu/trnsparse/tests/ -v -s -m neuron --tb=short"
else
  PYTEST_INVOCATION="\$NEURON_VENV/bin/pytest /home/ubuntu/trnsparse/tests/ -v -m neuron --tb=short"
fi
...
CMD_ID=$(aws ssm send-command \
  ...
  --comment "trnsparse neuron tests @ $SHA" \
  --parameters "commands=[
    \"bash -c 'set -euo pipefail; cd /home/ubuntu/trnsparse && sudo -u ubuntu git fetch --all && sudo -u ubuntu git checkout $SHA && NEURON_VENV=\$(ls -d /opt/aws_neuronx_venv_pytorch_* | head -1) && sudo -u ubuntu \$NEURON_VENV/bin/pip install -e /home/ubuntu/trnsparse[dev] --quiet && sudo -u ubuntu $PYTEST_INVOCATION'\"
  ]" \
  --region "$REGION" \
